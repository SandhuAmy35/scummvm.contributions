...
IFXRESULT CIFXRenderContext::SetWindow(IFXRenderWindow& rWindow)
{
	...
	if(m_ppRender[m_uCurrentDevice].IsValid())
	{
		...
		if (IFXFAILURE(rc)) printf("m_ppRender->SetWindow failed with 0x%08x\n", rc);
	}
	...
	if(IFX_E_NEED_RESET == rc)
	{
		if(!bInReset)
		{
			...
			if (IFXFAILURE(rc)) printf("GetNumDevices failed with 0x%08x\n", rc);

			if(IFXSUCCESS(rc))
			{
				rc = SetWindow(rWindow);
				if (IFXFAILURE(rc)) printf("Inner SetWindow failed with 0x%08x\n", rc);
			}
		}
		else
		{
