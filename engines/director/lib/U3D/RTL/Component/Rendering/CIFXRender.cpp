...
IFXRESULT CIFXRender::SetWindow(IFXRenderWindow& rWindow)
{
	...
	else printf("CIFXRender::SetWindow: m_iInitialized failed with 0x%08x\n", rc);

	if(IFXSUCCESS(rc))
	{
		rc = SetHWWindow(bDirty);
		if(IFXFAILURE(rc)) printf("SetHWWindow failed with 0x%08x\n", rc);

		if(IFXSUCCESS(rc))
		{
			rc |= m_spDevice->SetWindowSize(rWindow.GetWindowSize());
			if(IFXFAILURE(rc)) printf("SetWindowSize failed with 0x%08x\n", rc);
			rc |= m_spDevice->SetVisibleWindow(m_rcVisibleWindow);
			if(IFXFAILURE(rc)) printf("SetVisibleWindow failed with 0x%08x\n", rc);
		}
	}
