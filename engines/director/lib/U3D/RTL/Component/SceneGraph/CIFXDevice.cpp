...
IFXRESULT CIFXDevice::Render()
{
	...
	if ( m_spRenderContext.IsValid() )
	{
		...
		if(IFXSUCCESS(result))
		{
			...
			while(uNumDevices--)
			{
				result = m_spRenderContext->SetDevice(uNumDevices);
				if (IFXFAILURE(result)) printf("SetDevice failed with 0x%08x\n", result);
				if (IFXSUCCESS(result)) {
					result = m_spRenderContext->SetWindow(m_RenderWindow);
					if (IFXFAILURE(result)) printf("SetWindow failed with 0x%08x\n", result);
				}
				...
				if(IFXSUCCESS(result))
				{
						...
						if(IFXSUCCESS(result))
						{
							if ( viewData.m_instance < viewInstanceCount )
							{
								...
								result = viewData.m_pView->RenderView( 
									...
									viewData.m_instance );
								if (IFXFAILURE(result)) printf("RenderView failed with 0x%08x\n", result);
							}
							else
							{
								result = IFX_E_INVALID_RANGE;
					...
					if (IFXSUCCESS(result)) {
						...
						if (IFXFAILURE(result)) printf("Swap failed with 0x%08x\n", result);
					}
			}
		}
	}
	...
	if (IFXFAILURE(result)) printf("CIFXDevice::Render failed with 0x%08x\n", result);
	return result;
}
